...
map(function(rangeValue){return rangeValue.replace(colorRegex,colorToRgba);}).
map(function(rangeValue){return rangeValue.replace(colorNamesRegex,colorToRgba);});
...
replace(stringShapeRegex,function(){return interpolations[i++](input);}).


replace(
/rgba\(([0-9\.-]+), ([0-9\.-]+), ([0-9\.-]+), ([0-9\.-]+)\)/gi,
function(_,p1,p2,p3,p4){return'rgba('+
Math.round(p1)+', '+Math.round(p2)+', '+Math.round(p3)+', '+p4+')';});
